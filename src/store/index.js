...
const Store = new Vuex.Store({
  ...
  actions: {
    fetchArticle: (state, articleId) => {
      const request = axios.get(`/api/article/${articleId}`)
        .then((response) => {
    ...
    fetchArticles: (state, props) => {
      ...
      const request = axios.get(`/api/articles?limit=${limit}&offset=${offset}`)
        .then((response) => {
    ...
    fetchSimilarArticles: (state, props) => {
      const limit = props.limit ? props.limit : 10;
      const offset = props.offset ? props.offset : 0;
      const request = axios.get(`/api/article/${props.article_id}/similar?limit=${limit}&offset=${offset}`)
        .then((response) => {
    ...
    fetchArticlesBySearch: (state, props) => {
      const limit = props.limit ? props.limit : 10;
      const offset = props.offset ? props.offset : 0;
      const request = axios.get(`/api/articles/search?query=${props.query}&limit=${limit}&offset=${offset}`)
        .then((response) => {
